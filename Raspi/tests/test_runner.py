...
async def test_m_to_n():
    source1 = MockSource(name='source1', timestamps=mock_timestamps(start=datetime(2024, 3, 20, 12, 51), interval=timedelta(seconds=3)), temperature=36.5)
    have1_1, cond1 = have_n_samples(1)
    sink1 = MockSink(cond=cond1)
    source1.connect(sink1)

    source2 = MockSource(name='source2', timestamps=mock_timestamps(start=datetime(2024, 3, 20, 12, 52), interval=timedelta(seconds=3)), temperature=0.5)
